...
#    criterion=nn.NLLLoss2d(torch.FloatTensor(CLASS_WEIGHT)).cuda()
    ...
    for batch_idx,(data,target) in enumerate(train_loader):
        ...
        count += np.sum(pred == target)
        
        tar = 512*512*args.batch_size     

        wrong = np.ones(pred.shape)
        wrong[pred == target] = 0
...
def save_checkpoint(state, is_best,filename= ROOT +
            'unet_code/dice_lesion_crop/checkpoints/checkpoint_aug_de.pth.tar'):
    """
    save checkpoint 
    """
    ...
    if is_best:
        shutil.copyfile(filename,
                    ROOT+'unet_code/dice_lesion_crop/checkpoints/model_best_aug_de.pth.tar')
